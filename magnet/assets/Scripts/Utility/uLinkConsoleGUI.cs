...
[AddComponentMenu("MagnetGames/Console GUI")]
internal class uLinkConsoleGUI : MonoBehaviour
{
    ...
    public LogMask autoShowOnLogMask = new LogMask { message = false, warning = false, error = true };
...
#if UNITY_ANDROID
    public KeyCode showByKey = KeyCode.Escape;
    public KeyCode showByKey_01 = KeyCode.Menu;
#elif UNITY_IOS
    public KeyCode showByKey = KeyCode.Home;
    public KeyCode showByKey_01 = KeyCode.Menu;
#endif
    ...
    void Update()
    {
        if ((showByKey != KeyCode.None && Input.GetKey(showByKey)))
        {
        ...
        else if (showByKey != KeyCode.None && Input.GetKeyUp(showByKey))
        {
            DownTime = 0;
        }
        else if (showByKey_01 != KeyCode.None && Input.GetKeyDown(showByKey_01))
        {
            DownTime = 0;
            SetVisible(!_isVisible);
        }
    ...
    void OnGUI()
    {
        ...
        {
            if (GUI.Button( new Rect(0, 0, 100, 50), "DebugLog"))
            {
                SetVisible(!_isVisible);
            }
        }
    ...
    void DrawGUI()
    {
        ...
        stackTraceLabelStyle.fontSize = 20;
        ...
        foreach (var entry in drawEntries)
        {
            if (filterLogMask.Filter(entry.type))
            {
                ...
                GUI.changed = false;
                entry.expanded = GUILayout.Toggle(entry.expanded, entry.logSpaced, stackTraceLabelStyle);
                if (GUI.changed) autoScroll = false;
    ...
    void CaptureLog(string log, string stacktrace, LogType type)
    {
        ...
        {
            //< 해당 이름이 들어가면 패스한다
            if (log.Contains("m_IsVisibleInScene") || log.Contains("dt >= 0") || log.Contains("psTemp.sheetIndex"))
                return;

            //< 제이슨 파일등을 날린다
            //JsonLoader.instance.AllDeleteData();

            //if (Application.platform != RuntimePlatform.WindowsEditor)
            //{
            //    if (type != LogType.Log && type != LogType.Warning)
            //    {
            //        if (!ErrorLogList.Contains(log + stacktrace))
            //        {
            //            ErrorLogList.Add(log + stacktrace);

            //            //< 서버로 에러를 전송
            //            //NetworkBase.instance.SendErrorData(log, stacktrace);
            //        }
            //    }
            //}
            ...
            UIHelper.ConsolLog(UIHelper.ConsolLogType.Error, "이상현상");
            //UIMgr.instance.OpenPopup("게임 진행중 이상현상이 발생하였습니다.\n게임이 종료되므로, 다시 실행해주세요.", () => 
            //{
            //    Application.Quit();
            //}, false);
        }
