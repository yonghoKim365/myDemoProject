...
public class EffectEditorSelector : ScriptableWizard
{
	...
	static public void Show<T> (OnSelectionCallback cb, string[] extensions) where T : Object
	{
		...
		string title = "Select a " + GetName(type);
		EffectEditorSelector editor = ScriptableWizard.DisplayWizard<EffectEditorSelector>(title);
		editor.mTitle = title;
		editor.mType = type;
		editor.mCallback = cb;
		editor.mExtensions = extensions;
		editor.mObjects = Resources.FindObjectsOfTypeAll(typeof(T));
		
		if (editor.mObjects == null || editor.mObjects.Length == 0)
		{
			editor.Search();
		}
		else
		{
			System.Array.Sort(editor.mObjects,
	...
	void Search ()
	{
		...
		if (mExtensions != null)
		{
			...
			{
				if(paths[i].Contains("08_windsoul_eff/00 data") == false)
				{
					continue;
				}
	...
	void OnGUI ()
	{
		...
		if (mObjects == null || mObjects.Length == 0)
		{
			...
			if (mType == typeof(EffectEditorData))
			{
				if (GUILayout.Button("Open the Effect Editor", GUILayout.Width(150f)))
				{
					EditorWindow.GetWindow<EffectEditor>(false, "EffectEditor", true).Show();
					isDone = true;
				}
			}
