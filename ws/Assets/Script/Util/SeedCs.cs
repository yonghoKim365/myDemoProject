...
namespace SeedCs
{
    public class SEED
    {
        ...
        public static byte[] Encrypt(byte[] Data, byte[] seedKey, bool cbcPad)
        {
            ...
            if (cbcPad)
            {
                int nPadding = nOutSize - nInSize;
                if (nPadding == 0) nPadding = 16;
                for (int i = nInSize; i < nOutSize; i++)
                {
                    OutData[i] = (byte)nPadding;
                }
            }
            ...
            byte[] PrevData = { 0xFE, 0x82, 0x43, 0xAA, 0x18, 0x17, 0x32, 0xCC, 0xFE, 0xDC, 0x12, 0x98, 0x75, 0x23, 0x32, 0xFE };
            ...
            for (int i = 0; i < nOutSize; i += 16)
            {
                ...
                if (cbcPad)
                {
                    XorByte(ref DataBlock, PrevData);
                }
                ...
                if (cbcPad)
                {
                    Buffer.BlockCopy(DataBlock, 0, PrevData, 0, 16);
                }
            }
            return OutData;
        }
        ...
        public static byte[] Decrypt(byte[] Data, byte[] seedKey, bool cbcPad)
        {
            ...
            {
                byte[] DataBlock = new byte[16];
                Buffer.BlockCopy(Data, i, DataBlock, 0, 16);
                ...
                if (cbcPad)
                {
                    Buffer.BlockCopy(Data, i, PrevData, 0, 16);
                }
            }

            if (cbcPad)
            {
                int nPadding = OutData[nOutSize - 1];
                if (nPadding > 0 && nPadding <= 16)
                {
                    int i = 0;
                    for (i = nOutSize - 1; i > nOutSize - nPadding; i--)
                    {
                        if (OutData[i] != nPadding) break;
                    }
                    if (i == nOutSize - nPadding)
                    {
                        nOutSize -= nPadding;
                        Buffer.BlockCopy(OutData, 0, Data, 0, nOutSize);
                        OutData = new byte[nOutSize];
                        Buffer.BlockCopy(Data, 0, OutData, 0, nOutSize);
                    }
                }
            }
            return OutData;
