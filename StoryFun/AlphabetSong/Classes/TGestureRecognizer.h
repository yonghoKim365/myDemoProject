...
public:
    ...
    virtual bool onGesturePanDouble(TGesturePan* gesture) { return false; };
...
class TGesturePinch : public TGesture
{
    ...
    CC_SYNTHESIZE_PASS_BY_REF(Vec2, _translation, Translation);
    CC_SYNTHESIZE(float, _prevDistance, PrevDistance);
    CC_SYNTHESIZE(float, _moveDistance, MoveDistance);
};
...
class TGestureRecognizer : public Ref
{
    ...
    CC_SYNTHESIZE(bool, _panDoubleEnabled, PanDoubleEnabled);
    ...
    CC_SYNTHESIZE_PASS_BY_REF(Vec2, _tapFingerMoveTolerance, TapFingerMoveTolerance);   // Movement farther than this tolerance will be ignored for tap
...
protected:
    ...
    virtual void firePanDoubleEvent(TGesturePan* gesture);
