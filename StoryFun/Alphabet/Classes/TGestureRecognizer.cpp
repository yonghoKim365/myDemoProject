...
#define GESTURE_RECOGNIZER_TAP_FINGER_MOVE_TOLERANCE                    Vec2(10,10)
...
bool TGestureRecognizer::init()
{
    ...
    _panEnabled = true;
    _panDoubleEnabled = false;
    _pinchEnabled = true;
    _rotationEnabled = true;
    _swipeEnabled = true;

    _tapFingerMoveTolerance = GESTURE_RECOGNIZER_TAP_FINGER_MOVE_TOLERANCE;
...
void TGestureRecognizer::firePanDoubleEvent(TGesturePan* gesture)
{
    _lastPanId = gesture->getID();
    if (_gestureHandler) _gestureHandler->onGesturePanDouble(gesture);
}
...
void TSimpleGestureRecognizer::onTouchMoved(Touch* touch, Event* event)
{
    if (_finger0 && touch->getID() == _finger0->getID())
    {
        ...
        // Update for tap/double tap
        if (_tapEnabled || _doubleTapEnabled)
        {
            if (_tapValid)
            {
                // Tap becomes invalid because of the movement of the finger
                Vec2 delta = _finger0->getDelta();
                if (fabsf(delta.x) > _tapFingerMoveTolerance.x || fabsf(delta.y) > _tapFingerMoveTolerance.y)
                    _tapValid = false;
            }
        }
        ...
        else if( _panDoubleEnabled && _finger1)
        {
            if (!_lastPan)
            {
                _lastPan = new TGesturePan();
    ...
    if (_finger0 && _finger1 && _finger0Moved && _finger1Moved)
    {
        ...
        log("fingersStartDistanceSq:%f, fingersDistanceSq:%f, fingersDistanceChange:%f", fingersStartDistanceSq, fingersDistanceSq, fingersDistanceChange);
...
//        if (_pinchEnabled && fingersDistanceChange > _pinchFingersDistanceChangeTolerance)
        if (_pinchEnabled )
        {
            ...
            if (!_lastPinch)
            {
                ...
                _lastPinch->setPrevDistance(fabsf(_finger0StartPt.distance(_finger1StartPt)));
                _lastPinch->setMoveDistance(0);
            }
            ...
            auto originDist = fabsf(_finger0StartPt.distance(_finger1StartPt));
            auto prevDist = _lastPinch->getPrevDistance();
            auto currentDist = fabsf(_finger0Pt.distance(_finger1Pt));



            auto moveDist = (currentDist-originDist)-prevDist;

            log("originDist :%f, prevDist :%f, currentDist :%f, moveDist:%f", originDist, prevDist, currentDist, moveDist);
//            if (currentDist < originDist)
//            {
//                moveDist *= -1;
//            }

            _lastPinch->setMoveDistance(currentDist - prevDist);
            _lastPinch->setPrevDistance(currentDist);
...
void TSimpleGestureRecognizer::onTouchEnded(Touch* touch, Event* event)
{
    if (_finger0 && touch->getID() == _finger0->getID())
    {
        ...
        if (!_finger1)
        {
            ...
            if ((_tapEnabled || _doubleTapEnabled) && _tapValid)
            {
                // Prev tap
