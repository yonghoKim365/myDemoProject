//
//  TraceTraceHiddenRivalLayer.cpp
//  CapcomWorld
//
...
TraceHiddenRivalLayer* TraceHiddenRivalLayer::instance = NULL;

TraceHiddenRivalLayer* TraceHiddenRivalLayer::getInstance()
{
    if (!instance)
        return NULL;
...
TraceHiddenRivalLayer::TraceHiddenRivalLayer(NpcInfo* npcInfo, int _rid, int _remainHp, int _maxHp, int _rivalLevel, int _limitTime, int _questID)
{
...
TraceHiddenRivalLayer::~TraceHiddenRivalLayer()
{
...
void TraceHiddenRivalLayer::InitUI()
{
...
//    string path = "ui/cha/";
    ...
    CCMenuItemImage *pSprBtn1 = CCMenuItemImage::create("ui/quest/trace/trace_btn_blue_a1.png", "ui/quest/trace/trace_btn_blue_a2.png", this, menu_selector(TraceHiddenRivalLayer::BtnCallback));
    pSprBtn1->setAnchorPoint( ccp(1,0));
    pSprBtn1->setPosition( ccp(size.width/2-10,0));    // size.width/5 * 0,0));
    pSprBtn1->setTag(40);
    
    CCMenuItemImage *pSprBtn2 = CCMenuItemImage::create("ui/quest/trace/trace_btn_red_a1.png", "ui/quest/trace/trace_btn_red_a2.png", this, menu_selector(TraceHiddenRivalLayer::BtnCallback));
...
/*
  ...
  */
    ...
    this->schedule(schedule_selector(TraceHiddenRivalLayer::timeCounter), 1.0f);
    ...
    refreshHiddenRivalHp(yy);
    refreshHiddenRivalMaxHp(yy);
    loadHitGauge();
    refreshHitGauge();
...
void TraceHiddenRivalLayer::timeCounter()
{
    ...
    if ((remainHp <= 0) || (limitTime <= curTime))
    {
        ...
        this->unschedule(schedule_selector(TraceHiddenRivalLayer::timeCounter));
        removeChildByTag(701, true);
...
const char* TraceHiddenRivalLayer::textAdjust(const char *input)
{
...
void TraceHiddenRivalLayer::refreshHiddenRivalHp(int yy)
{
    removeHiddenRivalHp();
    ...
    for (int scan=0; scan<length; scan++)
    {
        ...
        hiddenRivalHp[scan] = createNumber(number, ccp(accp(x), yy + accp(50.5f)), scale);
        this->addChild(hiddenRivalHp[scan], 15);
        CCSize size = hiddenRivalHp[scan]->getTexture()->getContentSizeInPixels();
        x -= size.width * scale - 1;
        hiddenRivalHp[scan]->setPosition(ccp(accp(x), yy + accp(50.5f)));
    }
}

void TraceHiddenRivalLayer::refreshHiddenRivalMaxHp(int yy)
{
    removeHiddenRivalMaxHp();
    bool skip = true;
    ...
    for (int scan=9; scan>-1; scan--)
    {
        ...
        if (skip)
            continue;
        hiddenRivalMaxHp[scan] = createNumber(number, ccp(accp(x), yy + accp(50.5f)), scale);
        this->addChild(hiddenRivalMaxHp[scan], 15);
        CCSize size = hiddenRivalMaxHp[scan]->getTexture()->getContentSizeInPixels();
        x += size.width * scale - 1;
    }
}

void TraceHiddenRivalLayer::removeHiddenRivalHp()
{
    for (int scan=0; scan<10; scan++)
    {
        if (hiddenRivalHp[scan] != NULL)
            this->removeChild(hiddenRivalHp[scan], true);
        hiddenRivalHp[scan] = NULL;
    }
}

void TraceHiddenRivalLayer::removeHiddenRivalMaxHp()
{
    for (int scan=0; scan<10; scan++)
    {
        if (hiddenRivalMaxHp[scan] != NULL)
            this->removeChild(hiddenRivalMaxHp[scan], true);
        hiddenRivalMaxHp[scan] = NULL;
    }
}

void TraceHiddenRivalLayer::loadHitGauge()
{
    char buffer[64];
...
void TraceHiddenRivalLayer::refreshHitGauge()
{
    float ratio = 0;
...
void TraceHiddenRivalLayer::ccTouchesBegan(cocos2d::CCSet* touches, cocos2d::CCEvent* event)
{
    
}

void TraceHiddenRivalLayer::ccTouchesEnded(cocos2d::CCSet* touches, cocos2d::CCEvent* event)
{
...
void TraceHiddenRivalLayer::BtnCallback(CCObject* pSender)
{
    soundButton1();
...
//                ResponseRivalBattle *battleResult = ARequestSender::getInstance()->requestRBattle(rid, PlayerInfo::getInstance()->traceTeam);
                
//                int r = atoi(battleResult->res);
//                if (r == 0){
...
//                    TraceLayer::getInstance()->setRivalBattleResult(battleResult);
...
//                else if (r == 2024){
                    ...
                    popupNetworkError(TraceLayer::getInstance()->responseRivalBattleInfo->res, TraceLayer::getInstance()->responseRivalBattleInfo->msg, "requestRBattle error");
                }
            }
            else{
...
void TraceHiddenRivalLayer::callBackHiddenRival()
{
    removeActionResouces();
    InitUI();
}

void TraceHiddenRivalLayer::removeActionResouces()
{
    for(int i=10;i<=20;i++){
...
void TraceHiddenRivalLayer::actionHiddenRival1()
{
    ...
    cbRival2 = CCCallFunc::create(this, callfunc_selector(TraceHiddenRivalLayer::actionHiddenRival2));
...
void TraceHiddenRivalLayer::actionHiddenRival2()
{
    for(int i=10;i<=20;i++){
        this->removeChildByTag(i, true);
    }
...
//    CCSprite* pSprBackground = CCSprite::create("ui/main_bg/bg_1.png");
    ...
    string charPath = CCFileUtils::sharedFileUtils()->getDocumentPath();
//    string path = "ui/cha/";
    charPath.append(npcInfo->npcImagePath);
    
    CCSprite* pSprEnemy = CCSprite::create(charPath.c_str());
    ...
    CCSprite* pSprHiddenRivalTitle = CCSprite::create("ui/quest/trace/quest_rival/quest_rival_hidden_1.png");
    pSprHiddenRivalTitle->setAnchorPoint(ccp(0.5f, 0.5f));
    pSprHiddenRivalTitle->setPosition(ccp(size.width/2.0f, size.height/2.0f));
    pSprHiddenRivalTitle->setOpacity(0.0f);
    pSprHiddenRivalTitle->setRotation(-45.0f);
    pSprHiddenRivalTitle->setScale(10.0f);
    pSprHiddenRivalTitle->setTag(14);
    
    CCActionInterval* hiddenRivalTitleFadeIn = CCFadeTo::create(7.0f / 30.0f, 255.0f);
    CCActionInterval* hiddenRivalTitleScaleDown = CCScaleTo::create(7.0f / 30.0f, 1.0f);
    CCActionInterval* hiddenRivalTitleRotate1 = CCRotateTo::create(3.0f / 30.0f, -34.5f);
    CCActionInterval* hiddenRivalTitleRotate2 = CCRotateTo::create((7.0f - 3.0f) / 30.0f, 0.0f);
    CCCallFunc* cbPlayHit = CCCallFunc::create(this, callfunc_selector(MyUtil::soundHit));
    CCActionInterval* hiddenRivalTitleDelay = CCDelayTime::create((66.0f - 7.0f) / 30.0f);
    CCActionInterval* hiddenRivalTitleStretch = CCScaleBy::create((72.0f - 66.0f) / 30.0f, 25.0f, 0.3f);
    CCActionInterval* hiddenRivalTitleFadeOut = CCFadeTo::create((72.0f - 66.0f) / 30.0f, 0.0f);
    
    CCCallFunc* cbRival2 = NULL;
    cbRival2 = CCCallFunc::create(this, callfunc_selector(TraceHiddenRivalLayer::callBackHiddenRival));
    
    pSprHiddenRivalTitle->runAction(CCSequence::create(CCSpawn::create(hiddenRivalTitleFadeIn, hiddenRivalTitleScaleDown, CCSequence::create(hiddenRivalTitleRotate1, hiddenRivalTitleRotate2, cbPlayHit, NULL), NULL), hiddenRivalTitleDelay, CCSpawn::create(hiddenRivalTitleStretch, hiddenRivalTitleFadeOut, NULL), cbRival2, NULL));
    
    layer->addChild(pSprHiddenRivalTitle, -9);
    //    this->addChild(pSprRivalTitle, -9);
    
    // 히든 라이벌 타이틀 효과
    
    CCSprite* pSprHiddenTitleEffect = CCSprite::create("ui/quest/trace/quest_rival/quest_rival_hidden_2.png");
    pSprHiddenTitleEffect->setAnchorPoint(ccp(0.5f, 0.5f));
    pSprHiddenTitleEffect->setPosition(ccp(size.width/2.0f, size.height/2.0f));
    pSprHiddenTitleEffect->setOpacity(0.0f);
    pSprHiddenTitleEffect->setTag(15);
    
    CCActionInterval* hiddenTitleEffectDelay = CCDelayTime::create(10.0f / 30.0f);
    CCActionInterval* hiddenTitleEffectFadeIn = CCFadeTo::create(0.0f, 255.0f);
    CCActionInterval* hiddenTitleEffectScaleUp = CCScaleBy::create((20.0f - 10.0f) / 30.0f, 3.0f);
    CCActionInterval* hiddenTitleEffectFadeOut = CCFadeTo::create((20.0f - 10.0f) / 30.0f, 0.0f);
    pSprHiddenTitleEffect->runAction(CCSequence::create(hiddenTitleEffectDelay, hiddenTitleEffectFadeIn, CCSpawn::create(hiddenTitleEffectScaleUp, hiddenTitleEffectFadeOut, NULL), NULL));
    
    layer->addChild(pSprHiddenTitleEffect, -9);
    ...
    CCSpriteFrame* thunderFrame0  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_01.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame1  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_02.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame2  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_03.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame3  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_04.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame4  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_05.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame5  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_06.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame6  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_07.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame7  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_08.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame8  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_10.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame9  = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_11.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame10 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_12.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame11 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_13.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame12 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_14.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame13 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_15.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame14 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_16.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame15 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_17.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame16 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_18.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame17 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_19.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame18 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_20.png", CCRectMake(0, 0, accp(640), accp(350)));
    CCSpriteFrame* thunderFrame19 = CCSpriteFrame::create("ui/quest/trace/quest_rival/quest_rival_lighten_21.png", CCRectMake(0, 0, accp(640), accp(350)));
    
    frameToThunder = new CCArray();
    frameToThunder->addObject(thunderFrame0);
    frameToThunder->addObject(thunderFrame1);
    frameToThunder->addObject(thunderFrame2);
    frameToThunder->addObject(thunderFrame3);
    frameToThunder->addObject(thunderFrame4);
    frameToThunder->addObject(thunderFrame5);
    frameToThunder->addObject(thunderFrame6);
    frameToThunder->addObject(thunderFrame7);
    frameToThunder->addObject(thunderFrame8);
    frameToThunder->addObject(thunderFrame9);
    frameToThunder->addObject(thunderFrame10);
    frameToThunder->addObject(thunderFrame11);
    frameToThunder->addObject(thunderFrame12);
    frameToThunder->addObject(thunderFrame13);
    frameToThunder->addObject(thunderFrame14);
    frameToThunder->addObject(thunderFrame15);
    frameToThunder->addObject(thunderFrame16);
    frameToThunder->addObject(thunderFrame17);
    frameToThunder->addObject(thunderFrame18);
    frameToThunder->addObject(thunderFrame19);
    
    CCSpriteFrame* thunderFrame = (CCSpriteFrame* )frameToThunder->objectAtIndex(0);
    spriteToThunder = CCSprite::createWithSpriteFrame(thunderFrame);
    spriteToThunder->setAnchorPoint(ccp(0.5f, 0.5f));
    spriteToThunder->setPosition(ccp(size.width/2.0f, size.height/2.0f));
    spriteToThunder->setOpacity(0.0f);
    spriteToThunder->setTag(17);
    
    CCAnimation* animationToThunder = CCAnimation::create();
    CCLog("frame count : %d", frameToThunder->count());
    for (int i=0; i<frameToThunder->count(); i++)
    {
        animationToThunder->addSpriteFrame((CCSpriteFrame* )frameToThunder->objectAtIndex(i));
    }
    animationToThunder->setLoops(1);
    animationToThunder->setDelayPerUnit((27.0f - 7.0f) / 30.0f / 15.0f);
    
    CCAnimate* animateToThunder = CCAnimate::create(animationToThunder);
    
    CCActionInterval* thunderDelay = CCDelayTime::create(7.0f / 30.0f);
    CCActionInterval* thunderShow = CCFadeTo::create(0.0f, 255.0f);
    CCActionInterval* thunderHide = CCFadeTo::create(0.0f, 0.0f);
    spriteToThunder->runAction(CCSequence::create(thunderDelay, thunderShow, animateToThunder, thunderHide, NULL));
    
    this->addChild(spriteToThunder, -10);
}